...
class Discriminator(nn.Module):
    def __init__(self, channel=6*32*4,lmax=6, out_class=1,is_dis =True):
        super(Discriminator, self).__init__()
        self.is_dis=is_dis
        self.channel = channel
        n_class = out_class 
        self.conv1 = nn.Conv3d(lmax, channel//8, kernel_size=4, stride=2, padding=1)
...
class Generator(nn.Module):
    def __init__(self, noise:int=100, lmax=6, channel:int=64):
        ...
        self.tp_conv5 = nn.Conv3d(_c, lmax, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, noise):
        ...
        h = F.interpolate(h,scale_factor = 1)
        h = self.tp_conv5(h)
